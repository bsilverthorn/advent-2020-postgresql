...
begin
    ...
    cross join (values (-1), (0), (1)) as wds(wd)
    group by 1, 2, 3, 4
    having
        count(case when xd <> 0 or yd <> 0 or zd <> 0 or wd <> 0 then true end) = 3
        or (
            count(case when xd <> 0 or yd <> 0 or zd <> 0 or wd <> 0 then true end) = 2
            and bool_or(xd = 0 and yd = 0 and zd = 0 and wd = 0)
        ) ;
...
create temp table cubes as select *, 0 as z, 0 as w from initial_cubes ;
